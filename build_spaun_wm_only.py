...
# with the WorkingMemory module in order to test the preprocessing and the 
# compute_stats functions. These functions and this file are used in this 
#branch to gather stats on SPAUN including synapse count, transform resource 
#count and encoding/decoding weights.
import os
import sys
import time
import argparse
...
from _spaun.spaun_main_wm_only import Spaun
...
def add_stubs_for_wm_model(model):
    with model:
        cnt_gate_input = nengo.Node(np.random.randn(1),label="cnt_gate_input")         
        nengo.Connection(cnt_gate_input, model.mem.cnt_gate_sig.input)              
                                                                                    
        mb1_reset_input = nengo.Node(np.random.randn(1),label="mb1_reset_input")       
        mb1_gate_input = nengo.Node(np.random.randn(1),label="mb1_gate_input")         
        nengo.Connection(mb1_reset_input, model.mem.mb1_reset)                      
        nengo.Connection(mb1_gate_input, model.mem.mb1_gate)                        
                                                                                    
        mb2_reset_input = nengo.Node(np.random.randn(1),label="mb2_reset_input")       
        mb2_gate_input = nengo.Node(np.random.randn(1),label="mb2_gate_input")         
        nengo.Connection(mb2_reset_input, model.mem.mb2_reset)                      
        nengo.Connection(mb2_gate_input, model.mem.mb2_gate)                        
                                                                                    
        mb3_reset_input = nengo.Node(np.random.randn(1),label="mb3_reset_input")       
        mb3_gate_input = nengo.Node(np.random.randn(1),label="mb3_gate_input")         
        nengo.Connection(mb3_reset_input, model.mem.mb3_reset)                      
        nengo.Connection(mb3_gate_input, model.mem.mb3_gate)                        
                                                                                    
        mbave_reset_input = nengo.Node(np.random.randn(1),label="mbave_reset_input")
        mbave_gate_input = nengo.Node(np.random.randn(1),label="mbave_gate_input")  
        nengo.Connection(mbave_reset_input, model.mem.mb_ave.reset)                 
        nengo.Connection(mbave_gate_input, model.mem.mb_ave.gate)                   
                                                                                    
        wm_module_in_node_input = nengo.Node(np.random.randn(cfg.sp_dim),label="wm_module_in_node_input")
        nengo.Connection(wm_module_in_node_input, model.mem.mem_in)                 
                                                                                    
        mb1_out = nengo.Node(step,size_in=cfg.sp_dim,label="mb1_out")               
        nengo.Connection(model.mem.mb1, mb1_out)                                    
        mb2_out = nengo.Node(step,size_in=cfg.sp_dim,label="mb2_out")               
        nengo.Connection(model.mem.mb2, mb2_out)                                    
        mb3_out = nengo.Node(step,size_in=cfg.sp_dim,label="mb3_out")               
        nengo.Connection(model.mem.mb3, mb3_out)                                    
                                                                                    
        mbave_out = nengo.Node(step,size_in=cfg.sp_dim,label="mbave_out")           
        nengo.Connection(model.mem.mb_ave.mem2.output, mbave_out)                   
                                                                                    
        prod_sys_input = nengo.Node(np.random.randn(1),label="prod_sys_input")         
        nengo.Connection(prod_sys_input,model.mem.mb1_reset)                        
        nengo.Connection(prod_sys_input,model.mem.mb1_gate)                         
        nengo.Connection(prod_sys_input,model.mem.sel_mb1_in.sel1)                  
        nengo.Connection(prod_sys_input,model.mem.mb1a_fdbk_gate.gate)              
        nengo.Connection(prod_sys_input,model.mem.mb1b_fdbk_gate.gate)              
                                                                                    
        nengo.Connection(prod_sys_input,model.mem.mb2_reset)                        
        nengo.Connection(prod_sys_input,model.mem.mb2_gate)                         
                                                                                    
        nengo.Connection(prod_sys_input,model.mem.mb3_reset)                        
        nengo.Connection(prod_sys_input,model.mem.mb3_gate)                         
        nengo.Connection(prod_sys_input,model.mem.sel_mb3_in.sel1)                  
        nengo.Connection(prod_sys_input,model.mem.sel_mb3_in.sel2)                  
        nengo.Connection(prod_sys_input,model.mem.mb3a_fdbk_gate.gate)              
        nengo.Connection(prod_sys_input,model.mem.mb3b_fdbk_gate.gate)              
        nengo.Connection(prod_sys_input, model.mem.mb_ave.gate)                     
        nengo.Connection(prod_sys_input, model.mem.mb_ave.reset)                    
                                                                                    
        nengo.Connection(prod_sys_input, model.mem.gate_sig_bias.input)             
    return model
